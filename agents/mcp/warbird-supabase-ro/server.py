...
"""Read-only Supabase REST MCP using anon/publishable key."""
...
mcp = FastMCP(
    "warbird_supabase_ro_mcp",
    instructions=(
        "Read-only Supabase REST select helper. Exposes GET/select only; no inserts, "
        "updates, deletes, RPC, or SQL execution."
    ),
)
...
@mcp.tool()
def supabase_ro_status(
    response_format: Literal["json", "markdown"] = "markdown",
) -> str | dict[str, Any]:
    """Report whether Supabase URL and anon/publishable key are available."""
    url = _env("NEXT_PUBLIC_SUPABASE_URL") or _env("SUPABASE_URL")
    key = (
        _env("NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY")
        or _env("SUPABASE_PUBLISHABLE_KEY")
    ...
    return _fmt(
        {
            ...
            "anon_or_publishable_key_present": bool(key),
...
def supabase_rest_select(
    ...
    """Read rows through Supabase REST with GET/select only."""
    ...
    if not url or not key:
        return _fmt({"ok": False, "error": "Supabase URL/key not available"}, response_format)

    try:
        filters = json.loads(filters_json or "{}")
    except Exception as exc:
        return _fmt({"ok": False, "error": "Invalid filters_json: " + repr(exc)}, response_format)
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "Warbird-Supabase-RO/1.0",
        },
    )
    ...
    try:
        ...
        payload = {"ok": False, "table": table, "error": repr(exc)}
