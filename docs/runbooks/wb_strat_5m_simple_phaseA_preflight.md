...
| R12 | Stage 4 export csv missing `nexus_signal_tier` because pine line 757 was display=none (TV doesn't export hidden plots) | High first time | High — parquet incomplete | Current checkpoint: `nexus_*` evidence plots stay `display.data_window` permanently; do not revert to `display.none`. Re-export after loading the updated Nexus Pine and verify `nexus_signal_tier` plus `nexus_fp_*` columns in the CSV header. |
