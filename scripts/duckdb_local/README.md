...
Renamed from `scripts/optuna/` on 2026-05-12. The V9 Core path is DuckDB-backed
AutoGluon and has no Optuna runtime dependency. Optuna methodology (and the
`optuna` Python library) is still used by the Nexus footprint research lane
and legacy v7 profile adapters retained in this directory.

## Layout

- `runner.py`
  Shared CLI for registry-backed indicator and strategy profile adapters
  (uses the `optuna` Python library for non-V9 HPO lanes).
- `warbird_optuna_hub.py`
  Card dashboard and child `optuna-dashboard` launcher (legacy support).
- `vscode_doctor.py`
...
- `workspaces/<indicator_key>/`
  Per-indicator canonical workspace home (DuckDB exports + manifests + study.db).
