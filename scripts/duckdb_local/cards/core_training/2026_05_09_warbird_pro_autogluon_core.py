...
from scripts.duckdb_local.paths import contract_study_db_path, study_db_path, workspace_dir

CARD_KEY = "warbird_pro_core"
CARD_TITLE = "2026-05-09 - Warbird Pro Autogluon Core"
PROFILE_MODULE = "scripts.duckdb_local.cards.core_training.2026_05_09_warbird_pro_autogluon_core"
...
# Locked 2026-05-11 training sequence: 15m first. The previous 5m default
# pointed at MES smoke artifacts under artifacts/v9_core_smoke_may2025/ and
# silently contradicted the locked rule.
DEFAULT_TIMEFRAME = "15"
DEFAULT_CSV = REPO_ROOT / "scripts/duckdb_local/workspaces/warbird_pro_core/exports/es_15m_core.csv"
DEFAULT_MANIFEST = DEFAULT_CSV.with_name("es_15m_core.manifest.json")
...
def _default_csv_path(symbol_root: str, timeframe: str) -> Path:
    return REPO_ROOT / "scripts/duckdb_local/workspaces/warbird_pro_core/exports" / f"{symbol_root.lower()}_{timeframe}m_core.csv"
...
def resolve_study_db_path(study_db: Path | None, manifest_path: Path) -> Path:
    ...
    timeframe = str(manifest.get("timeframe", "15")).strip() or "15"
