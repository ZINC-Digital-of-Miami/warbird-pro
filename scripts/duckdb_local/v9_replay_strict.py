...
Wrapper around the deprecated scripts/duckdb_local/v9_replay.py. Retired with the
...
raise SystemExit(
    "v9_replay_strict is DEPRECATED (Hybrid+ chain). See scripts/duckdb_local/cards/core_training/."
)
...
from scripts.duckdb_local import v9_replay as base


def main(argv: list[str]) -> int:
    parquet_path = Path(argv[1]) if len(argv) > 1 else Path("data/mes_5m.parquet")
    out_path = Path(argv[2]) if len(argv) > 2 else Path(
        "scripts/duckdb_local/workspaces/warbird_pro/exports/databento_mes_5m_2020-2026_strict.csv"
    )
