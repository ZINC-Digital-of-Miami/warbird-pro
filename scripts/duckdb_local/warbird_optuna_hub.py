...
What it does:
1) Reads indicator/strategy registry from `scripts/duckdb_local/indicator_registry.json`
...
# Ensure dotted profile modules like "scripts.duckdb_local.<name>" can resolve when
# this script is launched via absolute file path.
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

from scripts.duckdb_local.paths import WORKSPACES_ROOT, experiments_dir, workspace_dir
...
def build_run_command(spec: IndicatorSpec) -> str:
    ...
    if spec.profile_builtin:
        ...
        parts.append(f"--profile-module scripts.duckdb_local.{spec.key}_profile")
